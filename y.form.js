...
// 2023.03.20 - add option skip default header
// 2023.04.13 - add loadForm function and loadMaster call loadForm function
// 2023.04.17 - fix show wait
// 2023.04.27 - add locale options
// 2023.05.19 - fix element tableDom not exist
// 2023.05.23 - fix regex thousand separator
// 2023.06.05 - optimize download csv
// 2023.06.07 - fix download csv, fix prepare data xls
// 2023.06.19 - sort table (single field), refresh detail data when createButtonBack
//----------------------------------------------------------------------------------------------------------------------
...
(function (window, undefined) {
    ...
    class Form {
        constructor(param) {
            ...
            this.listenerHeaderTable();
            this.withoutSubmitButton = false;
        }

        async loadingScript(callbackAfterScriptLoaded) {
            showWait();
            const promises = [];
            const include = elvis(this.include, []);
            for (const script of include) {
                promises.push(this.includeScript(script))
            }
        ...
        superinit(doAfterSuperinit) {
            doAfterSuperinit = elvis(doAfterSuperinit, () => { });
            ...
            const doBeforeLoad = async () => {
                const promiseBeforeLoad = new Promise((resolve, reject) => {
                    this.beforeLoad(this)
                    resolve()
                })
                await promiseBeforeLoad.then((resBeforeLoad) => {
                    doLoad(resBeforeLoad)
                }, (err) => {
                    console.log('Error: function before load has error ' + err)
                })
            }
            const callbackAfterScriptLoaded = async () => {
                if (hasInitData) {
                    // with init data
                    await promiseGet(this.initDataUrl).then((resInitData) => {
            ...
            this.loadingScript(callbackAfterScriptLoaded)
        }
        ...
        listenerHeaderTable() {
            const that = this;
            const th = 'th.th-label.label_header';
            $('body').off('click', 'th.th-label.label_header.th-header-sortable');
            $('body').on('click', 'th.th-label.label_header.th-header-sortable', function (e) {
                e.preventDefault();
                const field = $(this).attr('col');
                const table = $(this).parent('tr').parent('thead').parent('table').attr('col');
                const thTable = `${th}.label_header_${table}`;
                $(thTable).removeClass('sort_ascending').removeClass('sort_descending');
                that.dataSorted[table] = that.dataSorted[table] ?? {};
                that.dataSorted[table]['order'] = that.dataSorted[table]['order'] ?? [];
                const currentOrder = that.getSortOrder(table, field);
                that.dataSorted[table]['original_data'] = that.dataSorted[table]['original_data'] ?? structuredClone(that.data[table]) ?? [];
                let data = structuredClone(that.dataSorted[table]['original_data']);
                switch (currentOrder) {
                    case 0:
                        // unsorted to ascending
                        $(this).addClass('sort_ascending');
                        that.setSortOrder(table, field, 1);
                        data = that.sortAscending(field, data);
                        that.setLastSortedData(table, data);
                        break;
                    case 1:
                        // ascending to descending
                        $(this).addClass('sort_descending');
                        that.setSortOrder(table, field, 2);
                        data = that.sortDescending(field, data);
                        that.setLastSortedData(table, data);
                        break;
                    case 2:
                        // ascending to unsorted
                        that.setSortOrder(table, field, 0);
                        that.setLastSortedData(table, data);
                        break;
                }
                that.updateTable(table, data);
            });
        }

        getSortOrder(table, field) {
            if (typeof this.field === 'undefined') return 0;
            if (typeof this.dataSorted === 'undefined') return 0;
            if (typeof this.dataSorted[table] === 'undefined') return 0;
            if (typeof this.dataSorted[table]['order'] === 'undefined') return 0;
            if (this.dataSorted[table]['order'] === null) return 0;
            for (const order of this.dataSorted[table]['order']) {
                if (order === null) continue;
                if (order.field === null) continue;
        ...
        setSortOrder(table, field, dir) {
            this.dataSorted = this.dataSorted ?? [];
            this.dataSorted[table] = this.dataSorted[table] ?? {};
            this.dataSorted[table]['order'] = this.dataSorted[table]['order'] ?? [];
            for (let i in this.dataSorted[table]['order']) {
                const order = this.dataSorted[table]['order'][i];
                if (order == null) continue;
                if (order.field == null) continue;
                if (order.field === field) {
                    if (dir > 0) {
                        this.dataSorted[table]['order'][i].dir = dir;
                    }
                    else {
                        this.dataSorted[table]['order'].splice(i, 1);
                    }
                    return;
                }
            }
            this.dataSorted[table]['order'].push({ 'field': field, 'dir': dir })
            return;
        }

        sortAscending(field, data) {
        ...
        setLastSortedData(table, data) {
            this.dataSorted[table] = this.dataSorted[table] ?? {};
            const last = this.dataSorted[table]['last_sorted_data'] ?? [];
            if (last.length == 0) {
                this.dataSorted[table]['original_data'] = this.data[table] ?? [];
            }
            this.dataSorted[table]['last_sorted_data'] = data;
        }

        resetSortTable(table) {
            this.dataSorted[table] = this.dataSorted[table] ?? {};
            this.dataSorted[table]['last_sorted_data'] = this.dataSorted[table]['last_sorted_data'] ?? [];
            this.dataSorted[table]['order'] = [];
            this.dataSorted[table]['original_data'] = null;
            const thTable = `th.th-label.label_header.label_header_${table}.th-header-sortable`;
            $(thTable).removeClass('sort_ascending').removeClass('sort_descending');
        }
            ...
            parent.addEventListener('keypress', function (e) {
                if (e.target && e.target.matches('textarea.textarea_single')) {
                    ...
                    if (key === 13) {
                        e.preventDefault();
                        if (data && data !== '') {
                            const nextNode = textarea.parentNode.parentNode.parentNode[isShift ? 'previousElementSibling' : 'nextElementSibling'];
                            if (nextNode !== null) {
                                const target = nextNode.querySelector(`textarea.input_${data}`);
                                target.focus();
                            }
                        }
                    } else if (key === 96) {
        ...
        listenerSelect() {
            const that = this
            $(document.body).off('click', 'tr>td.no')
            $(document.body).on('click', 'tr>td.no', function () {
        ...
        listenerSelect2Change() {
            $(document.body).off('select2:select')
            $(document.body).on('select2:select', function (e) {
                $(e.target).siblings('label').removeClass('active').addClass('active')
            })
        }
        selectedRowToRemove() {
            let count = 0
        ...
        listenerInputNumber(option) {
            ...
            //     input = input.replace(",", "");
            //     // if (!decimal) {
            //     //     const regexThousandSeparator = /,(?=[\d,]*\.\d{2}\b)/g;
            //     //     input = input.replace(",", "");
        ...
        listenerFilterTable(opt, callback) {
            ...
            $wrapper.off('click', '.btn-filter-' + table)
            $wrapper.on('click', '.btn-filter-' + table, function (e) {
                e.stopPropagation()
                e.stopImmediatePropagation()
                e.preventDefault()
                $('.row-y-filter-' + table).toggle()
            ...
            $wrapper.off('keypress', '.input-filter-' + table);
            $wrapper.on('keypress', '.input-filter-' + table, function (e) {
                const key = (e.keyCode ? e.keyCode : e.which);
                if (key == '13') {
                    e.stopPropagation();
                    e.stopImmediatePropagation();
                    e.preventDefault();
                    const data = that.filterTable(table);
                    that.updateTableDataSimple(table, data);
                    callback(data);
                }
            });

            $wrapper.off('change', '.input-filter-select-' + table)
            $wrapper.on('change', '.input-filter-select-' + table, function (e) {
                const data = that.filterTable(table);
                that.updateTableDataSimple(table, data)
                callback(data);
            });

            $wrapper.off('click', '.btn-filter-clear-' + table);
            $wrapper.on('click', '.btn-filter-clear-' + table, function (e) {
                e.stopPropagation();
                e.stopImmediatePropagation();
                e.preventDefault();
                $('.input-filter-' + table).val('');
                const data = that.data[table];
                that.updateTableDataSimple(table, data)
                callback(data)
            });
        }
        ...
        filterTable(table) {
            ...
            const data = structuredClone(this.data[table]) ?? structuredClone(this.data.master[table]) ?? [];
            const param = this.getFilterParam(fields);
            return param.length > 0 ? this.applyFilter(data, param) : data;
        }
        ...
        applyFilter(data, params = []) {
            ...
            for (const param of params) {
                ...
                rows = rows.filter(row => this.filterLike(row[field], val));
            }
            return rows;
        }
        ...
        filterLike(a, b) {
            if(b === '') return true;
            const c = a.toString().toLocaleLowerCase();
            const d = b.toString().toLocaleLowerCase();
            const e = d.substring(1);
            if (d.startsWith('=')) {
                return c === e;
            }
            if (d.startsWith('>')) {
        ...
        getValue(selector) {
            ...
            if (selector) {
                ...
                const regexThousandSeparator = /,(?=[\d,]*\.\d{2}\b)/g;
                const val_a = a ? a.replace(",", "") : false;
                const b = $(selector).text() || false;
                const val_b = b ? b.replace(",", "") : false;
        ...
        getValueLabel(field) {
            const element = document.getElementById('label_info_' + field);
            return typeof element !== 'undefined' && element !== null && typeof element.innerHTML !== 'undefined' ? element.innerHTML : false;
        }
        get_value_label(field) {
            this.getValueLabel(field)
        }
        setValue(obj, value, value2) {
            const doc = typeof obj === 'object' ? obj : document.querySelector(obj);
            if (doc === null) {
                return null;
            }
            value = typeof value !== 'undefined' ? parseFloat(value) : this.getValue(doc);
            const classList = doc.classList ?? doc[0].classList;
            const isSumHeader = classList.contains && classList.contains('sum-header-item') || $(obj).hasClass('sum-header-item');
            const isSumFooter = classList.contains && classList.contains('sum-footer-item') || $(obj).hasClass('sum-footer-item');

            // Recalculate from item
            if (isSumHeader || isSumFooter) {
                const fieldName = doc.getAttribute('data');
                const lastValue = typeof value2 !== 'undefined' ? value : this.getValue(doc);
                const currentTotal = isSumHeader
                    ? this.getValue('.header-sum-' + fieldName)
                    : this.getValue('.footer-sum-' + fieldName);
                const newValue = typeof value2 !== 'undefined' ? parseFloat(value2) : parseFloat(value);
                const newTotal = parseFloat(currentTotal) - parseFloat(lastValue) + newValue;
                if (isSumHeader) {
                    const sel = document.querySelector(`.header-sum-${fieldName}`);
                    this.setTextContent(sel, newTotal);
                }
                if (isSumFooter) {
                    const sel = document.querySelector(`.footer-sum-${fieldName}`);
                    this.setTextContent(sel, newTotal);
                }
            }
            // Direct
                ...
                if (type == 'input' || type == 'textarea' || type == 'select') {
                    ...
                    this.setTextContent(doc, value);
                }
            }
        }

        setTextContent(obj, valueContent) {
            const hasLocale = obj.getAttribute('locale');
            const locale = hasLocale !== null ? hasLocale : 'en-US';
            const hasLocaleOptions = obj.getAttribute('locale-options');
            const localeOptions = hasLocaleOptions !== null ? parseJSON(hasLocaleOptions) : {};
            obj.textContent = valueContent.toLocaleString(locale, localeOptions);
        }
        ...
        //---------------------------------------------------------------------
        // Recalculate sum header/footer table
        // Call this function everytime content/value is changed
        // To make coloumn has sum value:
        // - add option sumHeader or sumFooter at field table
        //---------------------------------------------------------------------
        recalculate(table, callback) {
            table = elvis(table, 'detail');
            callback = elvis(callback, () => { });
            const fieldTable = elvis(this.field[table], elvis(this.field.master[table], []));
            if (!fieldTable) {
                return false;
            }
            fieldTable
                .filter(field => elvis(field.sum_header, field.sumHeader))
                .forEach(field => {
                    this.calculateSumValue(table, elvis(field.name, false), 'header', field);
                });

            fieldTable
                .filter(field => elvis(field.sum_footer, field.sumFooter))
                .forEach(field => {
                    this.calculateSumValue(table, elvis(field.name, false), 'footer', field);
                });
            callback();
        }

        //---------------------------------------------------------------------
        // Specifically calculate a column in a table
        //---------------------------------------------------------------------
        calculateSumValue(table, name, type, field) {
            const that = this;
            ...
            if (typeof this.field.master !== 'undefined') {
                ...
                for (let i in master) {
                    ...
                    if (typeof field.table !== 'undefined' && field.table == true) {
                        ...
                        if (hasParam(field, 'isTitle')) {
                            this.titleField = field
                        }
                        if (hasParam(field, 'is_key') || hasParam(field, 'isKey')) {
                            this.keyField = field.name
                            if (hasParam(field, 'label_info') || hasParam(field, 'labelInfo')) {
                                this.keyFieldType = 'label'
                            } else {
                                this.keyFieldType = 'input'
                            }
                        }
                        if (hasParam(field, 'is_main') || hasParam(field, 'isMain')) {
                            ...
                            for (let j = parseInt(i) + 1; j < master.length; j++) {
                                ...
                                if (fieldNext) {
                                    const isLabelInfo = hasParam(fieldNext, 'label_info') || hasParam(fieldNext, 'labelInfo')
                                    if (!isLabelInfo) {
        ...
        activateLabel(selector) {
            selector = typeof selector !== 'undefined' ? selector : "input[id*='input'], select[id*='input']";
            $(selector).siblings('label').removeClass('active').addClass('active');
            $(selector).parent('div.select-wrapper').siblings('label').removeClass('active').addClass('active');
        }
        ...
        //---------------------------------------------------------------------
        loadForm(options) {
            options = elvis(options);
            const fields = elvis(options?.fields, []);
            const data = elvis(options?.data, {});
            const withRecalculate = elvis(options?.withRecalculate, true);
            const cais = elvis(options?.callbackAfterInitSelect, () => { });

            for (const field of fields) {
                const name = elvis(field.name);
                if (!name) continue;

                const isTable = elvis(field.table);
                let value = isTable
                    ? elvis(data[name], [])
                    : elvis(data[name], '');

                // Skip File
                const type = elvis(field.type);
                if (type === 'file') continue;

                // Skip Label Only
                const isLabelOnly = hasParam(field, 'label_only') || hasParam(field, 'labelOnly');
                if (isLabelOnly) continue;

                if (isTable) {
                    this.updateTable(name, value, cais);
                    if (withRecalculate) {
                        this.recalculate(name);
                    }
                }
                else {
                    const isTitle = hasParam(field, 'isTitle');
                    const isImage = hasParam(field, 'image');
                    const isRadio = hasParam(field, 'radio');
                    const isLabelInfo = hasParam(field, 'label_info') || hasParam(field, 'labelInfo');

                    // Handle format datetime
                    const format = elvis(field.format);
                    if (format == 'medium_datetime') {
                        value = y_datetime_convert(value, 'datetime_sql_to_medium_datetime');
                    }

                    // Handle title
                    if (isTitle) {
                        const label = elvis(field.label, '');
                        const title = label == '' ? label : `${label}: `;
                        yM.setTextValue('#master-title', `${title}${value}`);
                    }
        ...
        loadMaster(callbackAfterInitSelect) {
            ...
            if (dataMaster && field) {
                this.loadForm({
                    fields: field,
                    data: dataMaster,
                    withRecalculate: true,
                    callbackAfterInitSelect: callbackAfterInitSelect,
                });
                // for (let i in field) {
                // 	const name = typeof field[i].name !== 'undefined' ? field[i].name : false;
                // 	if (hasParam(field[i], 'table')) {
                // 		if (name && typeof dataMaster[name] !== 'undefined') {
                // 			if (!typeof this.data.master !== 'undefined') {
                // 			}
                ...
                // 		}
                // 		// Image
                // 		if (hasParam(field[i], 'image')) {
                // 			$('#preview-image-' + name).attr('src', value);
                // 			$('#title-preview-image-' + name).addClass("active");
                // 		}
                // 		if (hasParam(field[i], 'radio')) {
                // 			$('input[name=' + name + '][notvalue=' + value + ']').prop('checked', true);
                // 			if (value !== null) {
                // 				$('input[name=' + name + '][value=' + value + ']').prop('checked', true);
                // 			}
                ...
                // 			}
                // 		}
                // 		else if (hasParam(field[i], 'label_info') || hasParam(field[i], 'labelInfo')) {
                // 			yM.setTextValue('#label_info_' + name, value);
                // 		}
                // 		else if (!(hasParam(field[i], 'label_only') || hasParam(field[i], 'labelOnly'))) {
                ...
                // 			}
                // 		}
                // 		else if (hasParam(field[i], 'checkbox') || hasParam(field[i], 'checkBox')) {
                // 			yM.setInputValue('#input_' + name, value);
                // 		}
                // 		if (hasParam(field[i], 'isTitle')) {
        ...
        resetMaster(callback) {
            ...
            yM.setInputValue('.input-master-select');
            yM.setTextValue('.label_info');
        ...
        restoreTableMaster() {
            const tables = [];
            for (const field of this.field.master) {
                const name = field.name ?? '';
                if (elvis(field.table) && name !== '') {
                    tables.push(name);
                }
            }
            this.restoreTable(tables, true);
        }
        restoreTable(tables, isMaster = false) {
            ...
            for (const table of tables) {
                ...
                if (isMaster) {
                    this.data.master[table] = tempData;
                }
                this.data[table] = tempData;
        ...
        enableMasterInput(field) {
            const selector = typeof field !== 'undefined' ? '#input_' + field : '.input_master_text, .input-master-select';
            ...
            if (typeof field === 'undefined') {
                ...
                $('select.input-master-select').siblings('label').addClass('active');
            }
        }
        ...
        addRow(table = 'detail', i, data, last = false, triggerField, option = 'show') {
            ...
            switch (table) {
                ...
                case 'history':
                    this.data[table] = typeof this.data[table] !== 'undefined' ? this.data[table] : [];
                    this.data[table][i] = cloneObject(data);
                    break;
                default:
                    this.data[table] = typeof this.data[table] !== 'undefined' ? this.data[table] : [];
                    this.data[table][i] = cloneObject(data);
        ...
        /////////////////////////////////////////////////////////////////////////////////////
        // Write Table Simple
        // Write Table Simple with data from this.data[table]
        /////////////////////////////////////////////////////////////////////////////////////
        writeTableSimple(table = 'detail', callbackAfterInitSelect = () => { }) {
            const data = typeof this.data[table] !== 'undefined' ? this.data[table] : {};
            this.writeTableDataSimple(table, data, callbackAfterInitSelect);
        }

        /////////////////////////////////////////////////////////////////////////////////////
        // Write Table Simple
        // Write Table Simple with data from parameter
        /////////////////////////////////////////////////////////////////////////////////////
        writeTableDataSimple(table = 'detail', data, callbackAfterInitSelect = () => { }) {
            data = data ?? this.data[table] ?? [];
            const minRows = this.minRows;
            const length = typeof data.length !== 'undefined' ? data.length : 0;
            const rows = minRows + length;
            for (let i = 0; i < rows; i++) {
        ...
        /////////////////////////////////////////////////////////////////////////////////////
        // Write Table
        // Write Table with data from this.data[table]
        /////////////////////////////////////////////////////////////////////////////////////
        writeTable(table = 'detail', callbackAfterInitSelect = () => { }) {
            table = table ?? 'detail';
            const data = typeof this.data[table] !== 'undefined' ? this.data[table] : [];
            this.writeTableData(table, data, callbackAfterInitSelect);
        }
        write_table(table, callbackAfterInitSelect) { this.writeTable(table, callbackAfterInitSelect); }

        /////////////////////////////////////////////////////////////////////////////////////
        // Write Table Data
        // Write Table with data from parameter
        /////////////////////////////////////////////////////////////////////////////////////
        writeTableData(table = 'detail', data, callbackAfterInitSelect = () => { }) {
            data = data ?? this.data[table] ?? [];
            const minRows = this.minRows;
        ...
        showRows(table, end) {
            showWait()
            const tableShown = `${table}Shown`;
            const start = this.vars[tableShown];
            for (let i = start; i < end; i++) {
                const elements = document.querySelectorAll(`.panel_input_row_${table}[row="${i}"]`);
                if (elements.length > 0) {
                    elements.forEach(element => element.removeAttribute('hidden'));
                }
            }
            this.vars[tableShown] = end;
            this.showTableInfo(table);
            hideWait()
        }
        ...
        //---------------------------------------------------------------------
        updateTableData(table = 'detail', data, callbackAfterInitSelect = () => { }) {
            this.resetTableOnly(table);
            this.writeTableData(table, data, callbackAfterInitSelect);
            this.showTableInfo(table);
        }

        updateTableDataSimple(table = 'detail', data) {
            this.resetTableOnly(table);
            this.writeTableDataSimple(table, data);
        ...
        handleAjaxResponseTable(opt, res) {
            ...
            const callbackSuccess = typeof opt.callbackSuccess !== 'undefined' ? opt.callbackSuccess : (res) => {
                ...
                if (masterTable) {
                    this.data.master[table] = dataTable;
                }
                if (showAjaxResultSuccess) {
                    showAjaxResult([`success`], false, title);
                }
                if (simple) {
        ...
        resetTableOnly(table) {
            const panelElements = document.querySelectorAll(`.panel_y_${table}_table_data`);
        ...
        listenerButtonRemoveRow(options, callbackRow, callbackHeader) {
            ...
            if (typeof options !== 'undefined' && typeof options === 'function') {
                ...
                removeTableRow = typeof options.removeTableRow !== 'undefined' ? options.removeTableRow : true;
            }
            else {
            ...
            const removeSingleRow = function (obj, fromMultiple) {
                ...
                if (removeTableRow) {
                    const table = $(obj).attr('data');
                    const trObj = $(obj).closest('tr');
            ...
            const removeMultipleRows = function (objButtonHeader, table) {
                ...
                callbackHeader(objButtonHeader, classRow, arrayRowIndex)
            ...
            $(this.wrapper).on('click', classRow, function (event) {
                ...
                const dataTable = $(this).attr('data');
                const currentRowIcon = that.getButtonIcon(this);
                if (currentRowIcon == iconRemove) {
                    ...
                    $(idConfirmRemove).on('click', function () {
                        ...
                        const inst = M.Toast.getInstance($(this).parent());
                        inst.dismiss();
                        removeSingleRow(buttonObj);
                        that.resetTableNumber(dataTable);
                        that.showTableInfo(dataTable);
                    })
                }
                else {
            ...
            $(this.wrapper).on('click', classHeader, function (event) {
                ...
                const dataTable = $(this).attr('data')
                const classRowTable = '.btn-remove-row-' + dataTable
                if (event.shiftKey) {
                    ...
                    if (currentHeaderIcon == iconCheckHeader) {
                        ...
                        if (noOfRowToBeDeleted > 0) {
                            ...
                            $(idCancelClear).on('click', function () {
                                const inst = M.Toast.getInstance($(this).parent())
                                inst.dismiss();
                                returnToDeleteSingleRow(objButtonHeader, dataTable)
                            })
                            $(idCancelRemove).off('click')
                            $(idCancelRemove).on('click', function () {
                                const inst = M.Toast.getInstance($(this).parent())
                            ...
                            $(idConfirmRemove).on('click', function () {
                                const inst = M.Toast.getInstance($(this).parent())
                                inst.dismiss();
                                const index = removeMultipleRows(objButtonHeader, dataTable)
                                let message = '#'
                                for (let i in index) {
                                    if (message !== '#') {
                                        message += ', #'
        ...
        listenerButtonPrint(options, field, callbackRow, callbackHeader) {
            ...
            if (typeof options !== 'undefined' && typeof options === 'object') {
                ...
                callbackRow = typeof options.callbackRow !== 'undefined' ? options.callbackRow : () => { };
                callbackHeader = typeof options.callbackHeader !== 'undefined' ? options.callbackHeader : () => { };
                skipDefaultHeader = typeof options.skipDefaultHeader !== 'undefined' ? options.skipDefaultHeader : false;
            }
            else {
                table = typeof options !== 'undefined' ? options : '';
        ...
        //---------------------------------------------------------------------
        submit() {
            const message = yStr('confirmationSubmit');
        ...
        //---------------------------------------------------------------------
        // Callback after submit is complete
        //---------------------------------------------------------------------
        completeSubmit() {
            this.data = {};
            this.resetMaster();
        ...
        isNewSubmit() {
            if (typeof this.keyFieldType === 'undefined') return false;
            if (this.keyFieldType == 'input') return $('#input_' + this.keyField).val() === '';
            return $('#label_info_' + this.keyField).text() === '';
        }
        //---------------------------------------------------------------------
        // Post Form function
        //---------------------------------------------------------------------
        postForm(newMessage, editMessage, callback, extra) {
            ...
            extra = extra ?? false;
            newMessage = newMessage ?? yStr('confirmationRecordCreate');
            editMessage = editMessage ?? yStr('confirmationRecordEdit');
            const message = this.isNewSubmit ? newMessage : editMessage;
            const r = confirm(message);
            if (r === true) {
                this.submitForm(extra, callback);
            }
        }
        ...
        //---------------------------------------------------------------------
        // Get Form data form serialize input exclude filter
        //---------------------------------------------------------------------
        getFormData(formId, extra) {
            formId = formId ?? this.idForm;
            extra = extra ?? false;
            const data = this.serializeInputAsArrayExcludeFilter(formId);
            if (extra && typeof extra === 'string') {
                const extraObject = JSON.parse('{"' + decodeURI(extra.replace(/&/g, "\",\"").replace(/=/g, "\":\"")) + '"}');
        ...
        //---------------------------------------------------------------------
        submitForm(a, b, submitUrl) {
            const that = this;
            a = a ?? false;
            b = b ?? false;
            submitUrl = submitUrl ?? this.queryUrl + 'submit';
            const callback = b && typeof b === 'function' ? b : a && typeof a === 'function' ? a : false;
        ...
        prepareDataXls(name, sheetname) {
            const modeSelect = this.panel.modeSelect;
            ...
            rows.each(function () {
                ...
                res.data[index] = cell.map((i, val) => {
                    if ($(val).hasClass('td-input-select')) {
                        if (modeSelect == 'select2') {
                            const objS2 = $(val).children('div.input-field').children('select.input-select');
                            const dataS2 = objS2.length > 0 ? objS2.select2('data') ?? false : false;
                            return Array.isArray(dataS2) ? dataS2[0].text ?? '' : '';
                        }
                        else {
        ...
        createInputModal(param) {
            ...
            const labelButtonCancel = typeof param.labelButtonCancel !== 'undefined' ? param.labelButtonCancel : 'Cancel';
            const labelButtonClose = typeof param.labelButtonClose !== 'undefined' ? param.labelButtonClose : 'Close';
        ...
        initModalDatePicker(group, field) {
            ...
            $(document.body).off('click', '.datepicker-done');
            $(document.body).on('click', '.datepicker-done', function () {
        ...
        writeDetail(table, param, callback) {
            ...
            callback = typeof callback !== 'undefined' ? callback : (data) => {
                $('.panel-record-info-detail').show();
            ...
            return new Promise((resolve, reject) => {
                if (!scriptElement) {
                    ...
                    if (moduleSection) {
                        ...
                        script.src = `${origin}/js/include/${scriptName}`;
                        script.type = 'text/javascript';
                        script.id = scriptId;
                        script.onload = () => { resolve(`Script ${scriptId} has been loaded`) };
                        script.onerror = () => { reject(new Error(`Script ${scriptId} load error`)) };
                        moduleSection.appendChild(script);
                    } else {
                        reject(new Error('Element with id "module-section" not found'));
                    }
                } else {
                    resolve(`Element ${scriptId} already loaded`);
                }
            })
        }
    ...
    const getFields = function (object) {
        const result = [];
    ...
    const downloadCsv = function (fields, data, filename, regional, confirmation = 'Download ?', qs = '"') {
        fields = typeof fields != 'object' ? JSON.parse(fields) : fields;
        data = typeof data != 'object' ? JSON.parse(data) : data;
        const head = data[0] ?? [];
        filename = filename ?? "file_download.csv";
        regional = regional ?? "english";
        if (data === null) return;
        ...
        const eof = '\r\n';
        const qe = qs + separator;
        const isNumber = [];
        // Row title
        let label = '';
        for (const field of fields) {
            const isHidden = elvis(field.hidden);
            if (isHidden) continue;
            const name = field.name ?? '';
            if (name == '') continue;
            const fLabel = field.label ?? '';
            const isMulticolumn = elvis(field.multicolumn);
            const format = field.format ?? '';
            isNumber.push(format == 'number');
            if (isMulticolumn) {
                const colName = field.name ?? false;
                if (colName) {
                    const obj = head[colName] ?? [];
                    const titles = obj ? getFields(obj) : [];
            ...
            for (const field of fields) {
                ...
                if (isMulticolumn) {
                    ...
                    if (colName && colName == name) {
                        const obj = head[name] ?? false;
                        const titles = obj ? getFields(obj) : [];
                        for (const title of titles) {
                            const colData = cellData[title] ?? '';
                            const content = isNumber[k] ? y_format_currency(colData, '', regional) : colData;
                            row += qs + content + qe;
                            k++;
                        }
                    }
                ...
                const cellData = rowData[name] ?? '';
                const content = isNumber[k] ? y_format_currency(cellData, '', regional) : cellData;
                row += qs + content + qe;
                k++;
            }
            const rowText = row.slice(0, (row.length) - 1);
            csv += rowText + eof;
        }
