...
(function (window, undefined) {
    ...
    class Form {
        ...
        listenerFilterTableWithAjax(opt) {
            ...
            opt.showAjaxResultSuccess = false
            const that = this;
        ...
        recalculate(table, callback) {
            ...
            callback = elvis(callback, function () { });
            const fieldTable = elvis(this.field[table]);
        ...
        showData(isDetailPopulated, isHistoryPopulated, showSubmitButton, callbackAfter, callbackAfterInitSelect, callbackAfterMaster) {
            ...
            // const highlightDetail = function() {
        ...
        setRequery(table, controller) {
            ...
            this.requeryCallback[table] = (obj) => {
                ...
                const param = this.serializeInputFilter(table) + '&offset=' + length
                const callback = (res) => {
        ...
        showTableInfo(table) {
            ...
            if (length == 0) {
                ...
                const dataLength = typeof data.length !== 'undefined' ? data.length : 0
                const rows = minRows + dataLength
                const maxShownRows = rows < 40 ? rows : 40
        ...
        handleAjaxResponseTable(opt, res) {
            ...
            const simple = typeof opt.simple !== 'undefined' ? opt.simple : true
            ...
            const callbackError = typeof opt.callbackError !== 'undefined' ? opt.callbackError : (error, res) => {
                y_show_ajax_result(error, false, title)
            ...
            const callbackSuccess = typeof opt.callbackSuccess !== 'undefined' ? opt.callbackSuccess : (res) => {
                ...
                if (showAjaxResultSuccess) { y_show_ajax_result([`success`], false, title) }
                if (simple) {
            ...
            const callbackResError = typeof opt.callbackResError !== 'undefined' ? opt.callbackResError : () => {
                y_show_ajax_result(['Error: kesalahan response dari server'], false, title)
            }
        ...
        listenerButtonRemoveRow(options, callbackRow, callbackHeader) {
            ...
            const removeSingleRow = function (obj, fromMultiple) {
                ...
                $(trObj).remove()
                callbackRow(obj, classRow, fromMultiple)
                return rowIndex
            }
            const removeMultipleRows = function (objButtonHeader, table) {
                ...
                callbackHeader(objButtonHeader, classRow)
                return rowIndex
            }
            ...
            // listener button row
            if (table != '') {
                $(this.master).off('click', classRowAllTable)
            }

            $(this.master).off('click', classRow)
            $(this.master).on('click', classRow, function (event) {
            ...
            // listener header
            if (table != '') {
                $(this.master).off('click', classHeaderAllTable)
            }
            $(this.master).off('click', classHeader);
            $(this.master).on('click', classHeader, function (event) {
        ...
        listenerButtonCheck(options, field, callbackRow, callbackRowCheck, callbackRowUncheck, callbackHeader) {
            ...
            $(this.master).on('click', classHeader, function (event) {
                ...
                if (event.shiftKey) {
                    // Unselect all rows
                    $('.btn-check-row-' + table).each(function () {
                        const currentRowIcon = that.getButtonIcon(this)
                        callbackRow(this, classRow, currentRowIcon, fromMultiple)
